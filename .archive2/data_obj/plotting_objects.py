...
def check_palette_syntax(palette, table):
    ...
    palette = {cedarkit.utils.routing.paths.replace(palette_rel_word, rel_word): v for k, v in palette.items()}
    return palette
...
class LagPlot:
    ...
    def tidy_plot(self, legend=False, edge=True, bottom=True):
        ...
        self.ax.set_ylabel(cedarkit.utils.routing.paths.replace('rho_', 'ρ'))

        available_ylabel = self.ax.get_ylabel()
        available_ylabel = cedarkit.utils.routing.paths.replace('_', ' ')
