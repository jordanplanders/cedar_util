...
from cedarkit.utils.cli import get_parser, parse_flags
from cedarkit.utils.location_helpers import construct_convergence_name
from cedarkit.core.project_config import load_config
from cedarkit.utils.routing import get_real_output, get_weighted_flag, relationship_filter, rel_reformat, get_surrogate_output
from archive2 import decide_file_handling
...
def process_group_workflow(arg_tuple):
    ...
    if 'convergence_interval' in conv_match_d:
        if type(conv_match_d['convergence_interval']) == str:
            try:
                conv_match_d['convergence_interval'] = ast.literal_eval(
                    cedarkit.utils.routing.paths.replace('np.float64', ''))
            except Exception as e:
